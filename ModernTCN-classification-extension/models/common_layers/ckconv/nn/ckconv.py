...
import torch.fft
import torch.nn
import models.common_layers.ckconv as ckconv
import models.common_layers.ckconv.nn.functional as ckconv_F
from models.common_layers.ckconv.utils.grids import linspace_grid
...
class CKConv(CKConvBase):
    ...
    def forward(self, x):
        ...
        # print(f"kernel shape: {conv_kernel.shape}, self.separable: {self.separable}, causal: {self.causal}")
        # 4. Compute convolution & return result
        return self.conv(x, conv_kernel, self.bias, separable=self.separable, causal=self.causal)
