...
def get_conv1d(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
    # print(f"Conv1d params: in_ch={in_channels}, out_ch={out_channels}, kernel={kernel_size}, groups={groups}")
...
class Block(nn.Module):
    def __init__(self, large_size, small_size, dmodel, dff, nvars, small_kernel_merged=False, drop=0.1, use_convffn2=True):
        ...
        # print(f"nvars: {nvars}")
        # print(f"Computing channels: {nvars} * {dmodel} = {nvars * dmodel}")
...
class ModernTCN(nn.Module):
    ...
    def forward(self, x, padding_mask=None, attn_mask=None, tau=None):
        # print(f"Input shape: {x.shape}")  # Should be [batch_size, features, seq_len]
        if self.task_name == 'classification':
